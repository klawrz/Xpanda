...
class RephraseHUD {
    ...
    private func caretPosition() -> NSPoint? {
        ...
        guard AXUIElementCopyAttributeValue(focusedEl, kAXSelectedTextRangeAttribute as CFString, &rangeRaw) == .success,
        ...
        // Reject zero/degenerate rects — Chrome web content often returns CGRect.zero
        // for contenteditable / web form fields, which maps to an invalid screen position.
        guard rect.width > 0 || rect.height > 0 else { return nil }

        guard NSScreen.main != nil else { return nil }

        let flippedY = NSScreen.screens[0].frame.height - rect.maxY
        let point = NSPoint(x: rect.minX, y: flippedY)

        // Ensure the point actually falls on a screen; web views sometimes return
        // coordinates in a different space that ends up off all screens.
